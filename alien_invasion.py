...
class AlienInvasion:
	...
	def _update_bullets(self):
		"""Обновление позиции снарядов и уничтожение старые снаряды"""
	...
	def _update_aliens(self):
		...
		#Проверить, добрались пришельцы до нижнего края экрана
	...
	def _check_aliens_bottom(self):
		"""Проверьте добрались ли пришельцы до нижнего края"""
		screen_rect = self.screen.get_rect()
		for alien in self.aliens.sprites():
			if alien.rect.bottom >= screen_rect.bottom:
				#Происходит тоже что и со столкновение с кораблем
				self._ship_hit()
				break
	...
	def _change_fleet_direction(self):
		"""Опускает ыесь флот и меняет направление флота"""
		for alien in self.aliens.sprites():
